...
describe("Transaction Feature", () => {
  beforeEach(() => {
    transactionManager.discard(); // reset any active transaction
  });

  test("MULTI queues commands and EXEC runs them", async () => {
    expect(await routeCommand({ command: "MULTI", args: [] })).toBe("OK");

    expect(await routeCommand({ command: "SET", args: ["testkey", "123"] })).toBe("QUEUED");
    expect(await routeCommand({ command: "INCR", args: ["testkey"] })).toBe("QUEUED");
  ...
  test("DISCARD cancels queued transaction", async () => {
    expect(await routeCommand({ command: "MULTI", args: [] })).toBe("OK");
    expect(await routeCommand({ command: "SET", args: ["mykey", "value"] })).toBe("QUEUED");
