...
function sendCommands(socket, commands, onComplete) {
  ...
  const handleData = (chunk) => {
    ...
    for (let rawLine of split) {
      ...
      if (
        cleaned.length &&
        !cleaned.startsWith("Welcome to") &&
        !cleaned.startsWith("CACHEINVALIDATE")
      ) {
        lines.push(cleaned);
      }
    }
  ...
  const flatCommands = commands.map((cmd) =>
    Array.isArray(cmd) ? cmd.join(" ") : cmd
  );
  flatCommands.forEach((cmd) => socket.write(cmd + "\n"));
  ...
  test("should set and restrict ACL correctly", (done) => {
    const tempSocket = net.createConnection({ host: HOST, port: PORT }, () => {
      sendCommands(
        ...
        [
          "AUTH admin admin123",
          "ACL SETUSER demo on >demo +GET +SET +DEL +ACL +WHOAMI",
          "AUTH demo demo",
          "SET keyX value",
          "GET keyX",
          "DEL keyX",
